/**
 * BecomeAgentButton — вторичный CTA «Стать агентом» в шапке вкладки
 * «Мои объявления» (ADR-0140), рядом с «Разместить объявление».
 ...
 * бессмысленно, а агенту не нужно. В шапке портала кнопки нет намеренно:
 * предложение адресное и уместно там, где человек управляет своими
 * объявлениями, а не на каждой странице сайта.
 *
 ...
 *
 * Размер/классы приходят пропсами. `withHintText` дублирует подсказку строкой
 * под кнопкой: на тач-устройствах hover'а нет и `title` недостижим. Подсказка
 * занимает всю ширину (`w-full`), чтобы в flex-ряду действий переноситься на
 * отдельную строку, а не ужиматься между кнопками.
 */
'use client';
  ...
  if (application?.status === 'PENDING') {
    ...
    return (
      <>
        ...
        {withHintText && (
          <p className="mt-1.5 w-full text-center text-[12.5px] text-muted-foreground">
            {hint}
          </p>
        )}
      </>
