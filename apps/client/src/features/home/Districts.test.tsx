 ...
 * Проверяем: рендерятся ровно 8 центральных районов Ташкента, ссылки на
...
describe('Districts (курируемые районы Ташкента: чип с иконкой)', () => {
  it('рендерит 8 районов Ташкента; плитка ссылается на /search?district_id=, содержит svg-иконку и название', async () => {
    getLocale.mockResolvedValue('ru');
    const { container } = render(await Districts());

    // Ровно 8 плиток (сетка 4×2), все — ссылки на поиск по district_id.
    ...
    // Иконки во всех 8 плитках; размытых фото больше нет.
    expect(container.querySelectorAll('a svg')).toHaveLength(8);
