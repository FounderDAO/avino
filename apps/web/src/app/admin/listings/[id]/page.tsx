        ...
        <div className="col gap-20">
          <div className="a-card" style={{ padding: 22 }}>
            ...
            <div className="row gap-12 muted" style={{ fontSize: 14, marginTop: 8, flexWrap: 'wrap' }}>
              <span>{listing.type}</span><span>·</span><span>{listing.rooms} комн</span><span>·</span><span>{data?.bathrooms ?? '—'} с/у</span><span>·</span>{data?.parking_type && (<><span>{({ YARD: 'Двор', COVERED: 'Крытая', GARAGE: 'Гараж', UNDERGROUND: 'Подземная' } as const)[data.parking_type]}</span><span>·</span></>)}<span>{src.area ?? '—'} м²</span><span>·</span><span>{listing.district}</span>
            </div>
