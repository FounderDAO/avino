...
| `price_reduced` | boolean | «Цена снижена»: только объявления, у которых последнее изменение цены было снижением (в той же валюте). `false`/отсутствие — без фильтра. Работает во всех поисковых эндпоинтах (`/search`, `/search/bounds`, `/search/radius`, `/search/polygon`, `/search/clusters`, price-distribution) |
