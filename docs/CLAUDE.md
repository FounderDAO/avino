...
- **Multi-language listings**: объявление создаётся на одном языке; переводы на остальные (uz/ru/en) генерирует модератор вручную в очереди модерации (кнопка «Сгенерировать переводы» + правка), публикация (APPROVE→ACTIVE) заблокирована, пока нет всех языков (ADR-0091); все объявления проходят moderation queue NEW → ACTIVE/DRAFT/REJECTED/DELETED (см. §9).
...
Объявления:
- пользователь создаёт объявление на одном языке → статус NEW (в БД появляется только авторский перевод)
- переводы на остальные языки (uz/ru/en) генерирует МОДЕРАТОР ВРУЧНУЮ на этапе модерации
  (синхронная кнопка «Сгенерировать переводы» + построчная правка), а НЕ автоматически при
  создании (ADR-0091, заменил ADR-0025)
- опубликовать объявление (APPROVE → ACTIVE) нельзя, пока нет переводов на все языки uz/ru/en
...
Уровни моделей Claude (от мощного к быстрому):
- **Fable 5** — топ, «for your toughest challenges». ⚠️ Включён лимитированно
  (на момент записи — до 22 июня); если недоступен/платно — фолбэк на Opus.
...
```text
Fable 5 → самое сложное: крупная архитектура, тонкая отладка, ответственные
          ревью (пока доступен; иначе → Opus)
Opus    → планирование, архитектура, code review, сложная логика,
