...
Visual reference (UX Zillow hero — бренд Avino, палитра ADR-0060):

```text
- Hero-секция на всю ширину поверх фото-фона (дом/недвижимость),
  затемняющий overlay для читаемости текста.
- Крупный заголовок в 2 строки, белым, слева или по центру
  (напр. "Аренда. Покупка. Дома." — финальный текст утвердит Team Lead).
- По центру под заголовком — крупная строка поиска (высокая, скруглённая,
  на белом фоне): input с placeholder "Город, район или адрес"
  + кнопка-лупа (иконка lucide Search) справа внутри/рядом.
- Enter или клик по лупе → переход на страницу результатов с query-параметром.
- Mobile: заголовок меньше, строка поиска на всю ширину.
- Бренд: акцент-кнопка в primary-red (#e03c42), не синий Zillow.
- БЕЗ ипотеки / BuyAbility / Home Loans.
```
