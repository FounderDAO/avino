...
```text
feat/client-login-network-error-handling
```

Scope:

```text
Сейчас в LoginModal handleRequest/handleVerify ловят ошибку пустым catch {} —
при сбое fetch (сеть/CORS/таймаут, а не бизнес-ошибка с кодом) пользователь не
видит ничего: кнопка просто перестаёт грузиться, «бэкенд молчит». Показывать
внятный fallback-текст (errors.requestFailed/networkError) для FETCH_ERROR/нет кода,
и при желании кнопку «повторить». Только apps/client.
```
...
apps/client/src/store/api/apiError.ts
apps/client/messages/ (ru/uz/en: auth.errors.networkError)
```

Acceptance criteria:

```text
При FETCH_ERROR/отсутствии кода ошибки показывается человекочитаемый текст (i18n), а не тишина.
Пустые catch {} не глотают сетевые сбои молча.
Тексты добавлены для ru/uz/en.
Изменения только внутри apps/client.
```
