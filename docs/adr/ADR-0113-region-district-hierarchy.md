...
4. **Источник данных** — репозиторий `FounderDAO/uzbekistan-regions-data`:
   14 регионов + 210 районов Узбекистана. Сид встроен в миграцию
   `20260628120000_add_regions` (одна транзакция; `INSERT … ON CONFLICT DO NOTHING`
   для идемпотентности). UUID 12 ташкентских районов `d0000000-*` сохранены —
   данные не ломаются при накатке на базу с историческими листингами.
...
- Сид идемпотентен — безопасен для staging и CI.
