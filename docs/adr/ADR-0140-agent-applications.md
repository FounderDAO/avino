...
Отзыв роли агента админом существует (`DELETE /admin/users/:id/roles/:role`,
он просто удаляет строку `user_roles`), а запись `agent_applications`
навсегда оставалась `APPROVED`. Клиент `/become-agent` трактует
`APPROVED`-заявку как «вы уже агент» (фолбэк для гонки «роль ещё не
подтянулась после одобрения»), поэтому экс-агент попадал в тупик: страница
показывала «Вы уже агент», ссылка «Мой профиль агента» вела на 404 (из
каталога он уже выпал), а `POST` новой заявки UI не давал отправить.

Решение: `GET /users/me/agent-application` отдаёт `APPROVED`-заявку только
пока пользователь реально держит роль `AGENT`/`AGENCY`; иначе — `404`, как
будто заявок не было (история в БД сохраняется). Клиент без изменений: 404 он
уже мапит в «заявок нет» → форма подачи. Фолбэк гонки deep-link сохраняется —
сразу после approve роль в БД уже есть, и `GET` продолжает отдавать
`APPROVED`.
